            ...
            <div class="flex flex-col ">
                ...
                >
                            ...
                            <input
                                ...
                                :id="key"
                                class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                :class="[duplicates.includes(`${collection_detail.color.value}-${index}`) ? 'border-[1px] border-red-500' :'border-gray-300']"
                            ...
                            <p v-if="duplicateError && duplicates.includes(`${collection_detail.color.value}-${index}`)" class="text-sm font-medium text-red-500">{{ duplicateError }}</p>
                        </div>
                    </div>
            </div>
    ...
    methods: {
        ...
        validateCollectionDetails() {
            ...
            for (let i = 0 ; i <  this.collection_details.length ; i++) {
                if(!this.collection_details[i].color.value) return;
                const key = `${this.collection_details[i].color.value}`;
                const duplicateKey = `${this.collection_details[i].color.value}-${i}`;;
                if (seen.has(key)) {
            ...
            if (this.duplicates.length > 0) {
                this.duplicateError = `${this.collection_name} ${this.duplicates[0].split('-').slice(0,1).join('/')} is already exists.`;
            } else {
