<script setup>
import { usePage } from '@inertiajs/vue3';

import SideNav from '@/Components/Admin/SideNav.vue';
import HeaderNav from '@/Components/Admin/HeaderNav.vue';
import FlashMessages from '@/Components/Global/FlashMessages.vue';
import ConfirmationDialog from '@/Components/Molecules/ConfirmationDialog.vue';
import InformationDialog from '@/Components/Molecules/InformationDialog.vue';

const page = usePage();
</script>

<template>
    <div  v-if="page.component !=='Admin/LoginForm'"  class="flex h-screen overflow-hidden font-roboto">
        <SideNav />

        <div class="relative flex flex-1 flex-col overflow-y-auto overflow-x-hidden">
            <HeaderNav />

            <main>
                <div
                    class="mx-auto max-w-screen-2xl"
                    :class="{
                        'p-4 md:p-6 2xl:p-10 bg-gray-50': $page.component !== 'Admin/Dashboard',
                    }"
                >
                    <slot />
                </div>
            </main>
        </div>

        <FlashMessages/>
        <ConfirmationDialog/>
        <InformationDialog/>
    </div>
    <div v-else>
        <slot/>
    </div>
</template>
