...
<template>
    <div class="relative">
        <div
            class="flex w-full h-full justify-between font-semibold text-sm items-center link-wrapper transition duration-150 rounded-md hover:bg-yellow-700 hover:text-white focus-within:bg-gray-700 cursor-pointer pr-3"
        ...
        >
            ...
            <div ref="dropdownContent" class="nav rounded-md p-2">
                ...
                <SideNavItem v-for="subItem in item.subPaths" :key="subItem" :item="subItem"
                             :class="$page.url === subItem.path ? 'text-primary bg-lightBlue hover:bg-primary' : ''"
                />
            </div>
        </div>
    </div>
