...
<template>
    <aside
        ref="target"
        class="absolute left-0 top-0 z-[9999] flex h-screen w-[270px] flex-col overflow-y-hidden text-white bg-black duration-300 ease-linear lg:static transition-all lg:translate-x-0"
        :class="{
            ...
            <Link :href="route('home')" class="block w-full">
                ...
                >
                    <img
                        ...
                        class="size-10"
                    />
                    <h3 class="font-bold text-md text-center">Steall Vogue POS</h3>
                </div>
            </Link>
        </div>
        <!-- Sidebar Header -->
                ...
                <nav class="">
                    <div class="flex flex-col h-auto overflow-y-auto scroll pb-12">
                        <div v-for="(item, index) in sidenavLinks" :key="index" class="">
                            <SideNavItem v-if="!item.subPaths&&isAuthorized(item.allowedRoles)" :item="item" />
                            <SubSideNavItemDropdown v-else-if="item.subPaths&&isAuthorized(item.allowedRoles)" :item="item" />
                        </div>
                    </div>
                    <!-- logout -->
                </nav>
